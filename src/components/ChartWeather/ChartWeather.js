...
const ChartWeather = () => {
    ...
    const chartBackground = useCallback( ({ widthChart, ctx}) => {
     ...
     // let widthScroll = document.getElementById('chart-container').scrollWidth;
      if(currentScroll < (window.innerWidth + window.innerWidth/2 ))
      { 
      ...
      } else if( currentScroll>= (window.innerWidth + window.innerWidth/2 ) && currentScroll < (window.innerWidth + window.innerWidth/2 + 2*window.innerWidth)){
        setScroll(currentScroll);
